...
//  changeParam('defaultnumber',e.value);
...
/*function movePaginationAfterTable() {
  // Sélectionne la table
  const table = document.querySelector('table');
  
  // Vérifie que la table existe
  if (table) {
    // Sélectionne la pagination
    const pagination = document.querySelector('.pagination');
    
    // Vérifie que la pagination existe
    if (pagination) {
      // Insère la pagination après la table
      table.parentNode.insertBefore(pagination, table.nextSibling);
      // Ajoute un saut de ligne après la pagination pour la placer en dessous de la table
      const lineBreak = document.createElement("br");
      table.parentNode.insertBefore(lineBreak, pagination.nextSibling);
    }
  }
}*/
...
function createPagin(){
  ...
  setTimeout(function() {
    $.get(url, function (data) {
      ...
      //on deplace la pagination sous le tableau
      //movePaginationAfterTable();
